...
class ParentChatConversationScreen extends StatefulWidget {
  ...
  const ParentChatConversationScreen({super.key, required this.contact});
...
class _ParentChatConversationScreenState extends State<ParentChatConversationScreen> {
  ...
  void initState() {
    super.initState();
    _parentId = authRepository.currentUser?.uid;
    _loadMessages();
  }

  Future<void> _loadMessages() async {
    if (_parentId == null) return;
    final msgs = await chatLocalRepository.getMessages(widget.contact.teacherId, _parentId!);
    if (mounted) {
      setState(() {
        _messages = msgs;
        _isLoading = false;
  ...
  Future<void> _sendMessage() async {
    ...
    if (text.isEmpty || _parentId == null) return;
    ...
    await chatLocalRepository.sendMessage(widget.contact.teacherId, _parentId!, text);
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Parent of ${widget.contact.specialization}',
                    style: const TextStyle(
